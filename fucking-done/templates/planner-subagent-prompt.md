...
Template for spawning fd-planner agent. The agent contains all planning expertise - this template provides planning context only.
...
<downstream_consumer>
Output consumed by /fd:execute-phase
...
**From /fd:plan-phase (standard mode):**
```python
Task(
...
**From /fd:plan-phase --gaps (gap closure mode):**
```python
Task(
...
**Note:** Planning methodology, task breakdown, dependency analysis, wave assignment, TDD detection, and goal-backward derivation are baked into the fd-planner agent. This template only passes context.
